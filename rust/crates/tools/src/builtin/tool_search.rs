...
impl Tool for ToolSearchTool {
    ...
    fn description(&self) -> &str { "Search for available tools by keyword." }
    fn prompt(&self) -> &str {
        "Search for tools by keyword. Use `select:<tool-name>` for exact match."
    }
    fn search_hint(&self) -> Option<&str> { Some("find tools by keyword search") }
    ...
    async fn call(
        ...
        let mut scored: Vec<(&ToolSearchEntry, i32)> = self
            ...
            .map(|entry| {
                ...
                for tok in &tokens {
                    ...
                    else if name.contains(tok) { score += 50; }
                    if hint.contains(tok) { score += 30; }
                    if desc.contains(tok) { score += 10; }
                }
                (entry, score)
            })
            .filter(|(_, s)| *s > 0)
        ...
        if scored.is_empty() {
            return Ok(ToolResult::ok("No matching tools found. Try different keywords."));
        }
