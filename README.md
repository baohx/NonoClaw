...
- Rust 1.82+ (`cargo`), Node.js and npm. The installer checks these commands, runs `npm ci` when `package-lock.json` is present, and builds the `nonoclaw` Cargo package with `--locked`.
...
The installer copies the release binary rather than symlinking the Cargo target, stages the executable before replacement, requires `frontend/dist/index.html`, and finishes with `nonoclaw --version`. `NONOCLAW_HOME` controls runtime settings/sessions (default `~/.nonoclaw`); it is separate from the installed Web asset directory.
...
- Rust 1.82+（`cargo`）、Node.js 和 npm。安装器会检查命令；存在 `package-lock.json` 时使用 `npm ci`，并以 `--locked` 构建 `nonoclaw` package。
...
安装器复制 release 二进制，不链接 Cargo target；替换前先在目标目录暂存可执行文件，强制要求 `frontend/dist/index.html`，并以 `nonoclaw --version` 收尾。`NONOCLAW_HOME` 控制运行时设置与 sessions（默认 `~/.nonoclaw`），不等同于 Web 静态资源安装目录。
