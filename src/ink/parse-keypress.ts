  ...
  for (const token of tokens) {
    if (token.type === 'sequence') {
      ...
      if (inPaste) {
        ...
        // Detect non-bracketed multiline paste: text tokens with embedded
        // newlines alongside other content indicate pasted multi-line text.
        // Bare \r (keyboard Enter) and bare \n are excluded — they should
        // be processed as normal keystrokes.
        const value = token.value
        const hasNewlines = /[\r\n]/.test(value)
        const isBareLineEnding = value === '\r' || value === '\n'
        if (hasNewlines && !isBareLineEnding) {
          keys.push(createPasteKey(value))
        } else {
